...
export async function GET(
  ...
  // Pre-built static (non-variable, glyf-outline) Noto Sans TC, embedded whole
  // with subset:false. pdf-lib/fontkit's own subsetter corrupts CJK fonts
  // (CFF → broken charstrings; large TrueType → dropped glyphs), and harfbuzz
  // subsetting needs a wasm that won't bundle cleanly into the Vercel lambda.
  // Embedding the whole static font sidesteps every subsetter; it covers all
  // BMP Traditional Chinese and the file embeds once (~3.7 MB) per document.
  const fontBytes = await fs.readFile(
    path.join(process.cwd(), "assets", "NotoSansTC-static.ttf")
  );
